...
def calculate_classification_metrics(
    num_classes: int, y_true_one_hot: np.ndarray, y_pred_probs: np.ndarray
):
...
def calculate_object_detection_metrics(
    num_classes: int,
    y_true_bbox_labels: List[Tuple[tf.Tensor, tf.RaggedTensor]],
    y_pred: List[Tuple[tf.Tensor, tf.RaggedTensor]],
):
...
def calculate_fomo_metrics(
    num_classes: int, y_true_labels: List[int], y_pred_labels: List[int]
):
